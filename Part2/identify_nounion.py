...
def process(spark: SparkSession):
    ...
    hashtags_df = hashtags_df.withColumn("month", month("created_at")).withColumn(
        "day", dayofmonth("created_at")).withColumn("hour", hour("created_at"))
    hashtags_df = hashtags_df.select("month", "day", "hour", "like_count", "quote_count",
                                     "reply_count", "retweet_count", "text", "tickers", "tags")
    print("hashtags_df.show(10)")
    hashtags_df.show(10)

    cashtags_df = cashtags_df.withColumn("month", month("created_at")).withColumn(
        "day", dayofmonth("created_at")).withColumn("hour", hour("created_at"))
    cashtags_df = cashtags_df.select("month", "day", "hour", "like_count", "quote_count",
                                     "reply_count", "retweet_count", "text", "tickers", "tags")
    print("cashtags_df.show(10)")
    cashtags_df.show(10)
    ...
    hashtag_time_tickers_tags_df = hashtags_df.select(
        "month", "day", "hour", "tickers", "tags")

    cashtag_time_tickers_tags_df = cashtags_df.select(
        "month", "day", "hour", "tickers", "tags")
    ...
    print("hashtag_time_tickers_tags_df.show(30)")
    hashtag_time_tickers_tags_df.show(30)

    print("cashtag_time_tickers_tags_df.show(30)")
    cashtag_time_tickers_tags_df.show(30)
    # time_tickers_tags_explode_df = time_tickers_tags_df.select()

    print("Hashtag Time tickers tags df show november")
    # hashtag_time_tickers_tags_df.filter(col('month') != 10).filter(
    #     col('hour') == 3).show(30)

    # print("Cashtag Time tickers tags df show november")
    # cashtag_time_tickers_tags_df.filter(col('month') != 10).filter(
    #     col('hour') == 3).show(30)

    hashtags_df.createOrReplaceTempView("hashtag")
    hashtag_ticker_freq_df: DataFrame = spark.sql(
        "select tickers, count(*) as cnt from hashtag group by tickers order by cnt desc")
    print("hashtag_ticker_freq_df.show(10)")
    hashtag_ticker_freq_df.show(10)

    cashtags_df.createOrReplaceTempView("cashtag")
    cashtag_ticker_freq_df: DataFrame = spark.sql(
        "select tickers, count(*) as cnt from cashtag group by tickers order by cnt desc")
    print("cashtag_ticker_freq_df.show(10)")
    cashtag_ticker_freq_df.show(10)

    hashtag_all_tickers_df = hashtags_df.select(
        explode_outer("tickers").alias("exploded"))
    print("hashtag_all_tickers_df.show(30)")
    hashtag_all_tickers_df.show(30)

    cashtag_all_tickers_df = cashtags_df.select(
        explode_outer("tickers").alias("exploded"))
    print("cashtag_all_tickers_df.show(30)")
    cashtag_all_tickers_df.show(30)

    hashtag_time_tickers_explode_df = hashtag_time_tickers_tags_df.withColumn(
        "tickers", explode_outer("tickers"))
    print("hashtag_time_tickers_explode_df.show(30)")
    hashtag_time_tickers_explode_df.show(30)

    cashtag_time_tickers_explode_df = cashtag_time_tickers_tags_df.withColumn(
        "tickers", explode_outer("tickers"))
    print("cashtag_time_tickers_explode_df.show(30)")
    cashtag_time_tickers_explode_df.show(30)
