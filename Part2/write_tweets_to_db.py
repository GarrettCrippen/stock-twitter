...
def process(spark: SparkSession):
    ...
    hashtags_df = tweets_hashtag_identified_df.select("id", "author_id", "created_at", "public_metrics.*", "text", "tickers").withColumn(
        "tags", udf(extract_hash_tags, ArrayType(StringType()))("text"))

    cashtags_df = tweets_cashtag_identified_df.select("id", "author_id", "created_at", "public_metrics.*", "text", "tickers").withColumn(
        "tags", udf(extract_hash_tags, ArrayType(StringType()))("text"))
    ...
    write_db(union_df.withColumn("tickers", concat_ws(",", union_df.tickers)
                                 ).withColumn("tags", concat_ws(",", union_df.tags)), "tweets")
